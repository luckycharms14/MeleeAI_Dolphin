...
Renderer::Renderer()
{
	/***Added for MeleeAI***/
	{
		...
		shared_memory_object::remove("MeleeAI");
		melee_memory = managed_shared_memory{create_only, "MeleeAI", 1024};
		melee_memory.construct<long>("game_data")[MELEE_MEM_SIZE](0);
		melee_memory.construct<long>("melee_ai_contr")[19](0);
	}
	/***********************/
...
{
	/***Added for MeleeAI***/
	boost::interprocess::shared_memory_object::remove("MeleeAI");
	/***********************/
}
...
void Renderer::SwapImpl(u32 xfbAddr, u32 fbWidth, u32 fbStride, u32 fbHeight, const EFBRectangle& rc, float Gamma)
{
	...
	/***Added for MeleeAI***/
  p1_data = PowerPC::HostRead_U32(0x80453130);
  p2_data = PowerPC::HostRead_U32(p1_data + 0x08);
  
	raw_input[0] = PowerPC::HostRead_U8(0x8045310E); //P1 stock count
	raw_input[1] = PowerPC::HostRead_U8(0x80453F9E); //P2 stock count
	raw_input[2] = PowerPC::HostRead_U16(0x804530E0); //P1 percent
	raw_input[3] = PowerPC::HostRead_U16(0x80453F70); //P2 percent
	raw_input[4] = PowerPC::HostRead_U8(0x803F0E0A); //P1 character
	raw_input[5] = PowerPC::HostRead_U8(0x803F0E2E); //P2 character
	raw_input[6] = PowerPC::HostRead_U32(0x80453090); //P1 X coordinate
	raw_input[7] = PowerPC::HostRead_U32(0x80453094); //P1 Y coordinate
	raw_input[8] = PowerPC::HostRead_U32(0x80453F20); //P2 X coordinate
	raw_input[9] = PowerPC::HostRead_U32(0x80453F24); //P2 Y coordinate
  raw_input[10] = PowerPC::HostRead_U16(0x804D6CAD); //Stage ID
  raw_input[11] = PowerPC::HostRead_U16(0x8046B6CC); //Frame Counter
  raw_input[12] = PowerPC::HostRead_U32(p2_data + 0x23A0); //Hitstun
  raw_input[13] = PowerPC::HostRead_U32(p2_data + 0xF0); //Vert Speed
  raw_input[14] = PowerPC::HostRead_U32(p2_data + 0x70); //Action State
  raw_input[15] = PowerPC::HostRead_U32(p2_data + 0x19BC); //Hitlag
  raw_input[16] = PowerPC::HostRead_U8(p2_data + 0x19C8); //Jumps Used
  raw_input[17] = PowerPC::HostRead_U32(p2_data + 0x19F8); //Shield Size
  raw_input[18] = PowerPC::HostRead_U32(p2_data + 0x140); //1 if P2 in air

	std::pair<long*, std::size_t> p1 = melee_memory.find<long>("game_data");
	for (int i = 0; i < MELEE_MEM_SIZE; ++i)
	{
		*(p1.first++) = raw_input[i];
	}

  std::pair<long*, std::size_t> p2 = melee_memory.find<long>("melee_ai_contr");
  for (int i = 0; i < 19; ++i)
  {
		melee_ai_output[i] = *(p2.first++);
	}

  PowerPC::HostWrite_U32(melee_ai_output[0], 0x80003F00);
  PowerPC::HostWrite_U32(melee_ai_output[1], 0x80003F04);
  PowerPC::HostWrite_U32(melee_ai_output[2], 0x80003F08);
  PowerPC::HostWrite_U8(melee_ai_output[3], 0x80003F18);
  PowerPC::HostWrite_U8(melee_ai_output[4], 0x80003F19);
  PowerPC::HostWrite_U8(melee_ai_output[5], 0x80003F1A);
  PowerPC::HostWrite_U8(melee_ai_output[6], 0x80003F1B);
  PowerPC::HostWrite_U8(melee_ai_output[7], 0x80003F1C);
  PowerPC::HostWrite_U8(melee_ai_output[8], 0x80003F1D);
  PowerPC::HostWrite_U8(melee_ai_output[9], 0x80003F1E);
  PowerPC::HostWrite_U8(melee_ai_output[10], 0x80003F1F);
  PowerPC::HostWrite_U32(melee_ai_output[11], 0x80003F20);
  PowerPC::HostWrite_U32(melee_ai_output[12], 0x80003F24);
  PowerPC::HostWrite_U32(melee_ai_output[13], 0x80003F28);
  PowerPC::HostWrite_U32(melee_ai_output[14], 0x80003F2C);
  PowerPC::HostWrite_U32(melee_ai_output[15], 0x80003F30);
  PowerPC::HostWrite_U32(melee_ai_output[16], 0x80003F34);
  PowerPC::HostWrite_U32(melee_ai_output[17], 0x80003F38);
  PowerPC::HostWrite_U32(melee_ai_output[18], 0x80003F3C);
